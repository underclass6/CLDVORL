...
def vanilla(args):
    ...
    logger = CompositeLogger(log_path=f"./results/awac/vanilla", name=exp_name, loggers_config={
    ...
    for i_epoch in trange(1, args.max_epoch + 1):
        ...
        if i_epoch % args.save_interval == 0:
            logger.log_object(name=f"policy_{i_epoch}.pt", object=policy.state_dict(),
                              path=f"./models/awac/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/seed{args.seed}/policy")
...
def without_CL(args):
    ...
    logger = CompositeLogger(log_path=f"./results/awac/withoutCL", name=exp_name, loggers_config={
        "FileLogger": {"activate": not args.debug},
        "TensorboardLogger": {"activate": not args.debug},
...
def traj_valuation(args):
    ...
    if args.run_dcla_ratio:
        logger = CompositeLogger(
            log_path=f"./ablation_results/awac/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/dcla_ratio/{args.dcla_ratio}",
            name=f"awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/awac/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/modify_ratio/{args.modify_ratio}/",
            name=f"awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/awac/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/std_scale/{args.std_scale}/",
            name=f"awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
    ...
    def valuate_traj(traj, policy_source, policy_target):
        ...
        #
        # std_scale = 0.1
        # act_mean_source, *_ = policy_source.actor.sample(obs)
        ...
        # act_mean_target, *_ = policy_target.actor.sample(obs)
    ...
    if args.run_dcla_ratio:
        np.save(logger.log_path + f"/avg_rewards_awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}.npy", avg_rewards)
    elif args.run_modify_ratio:
        np.save(
            logger.log_path + f"/avg_rewards_awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}.npy",
            avg_rewards)
    elif args.run_std_scale:
        np.save(
            logger.log_path + f"/avg_rewards_awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}.npy",
            avg_rewards)
...
def main(pass_in=None):
    args = parse_args('./reproduce/awac/config/cldv/base.py')
    ...
    for sd, td in zip(source_datasets, target_datasets):
        arguments[(sd, td)] = parse_args('./reproduce/awac/config/cldv/base.py')
    ...
    if run_exp['dcla_ratios']:
        for dr in dcla_ratios:
            for sd, td in zip(source_datasets, target_datasets):
                ...
                print(f'modify_ratio: {args.modify_ratio}')
