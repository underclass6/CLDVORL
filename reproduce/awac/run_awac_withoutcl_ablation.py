...
def without_CL(args):
    ...
    if args.run_dcla_ratio:
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/awac/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/filter_ratio/{args.filter_ratio}/",
            name=f"awac_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_filter_ratio_{args.filter_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
