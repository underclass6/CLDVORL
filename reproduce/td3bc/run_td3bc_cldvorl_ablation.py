...
def vanilla(args):
    ...
    logger = CompositeLogger(log_path=f"./results/td3bc/vanilla", name=exp_name, loggers_config={
    ...
    source_buffer, target_buffer = get_combined_buffer(env, source_buffer_o, target_buffer_o, ratio=1.0, device=args.dev)
    ...
    for i_epoch in trange(1, args.max_epoch + 1):
        ...
        if i_epoch % args.save_interval == 0:
            logger.log_object(name=f"policy_{i_epoch}.pt", object=policy.state_dict(),
                              path=f"./models/td3bc/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/seed{args.seed}/policy")
...
def without_CL(args):
    ...
    logger = CompositeLogger(log_path=f"./results/td3bc/withoutCL", name=exp_name, loggers_config={
        "FileLogger": {"activate": not args.debug},
        "TensorboardLogger": {"activate": not args.debug},
        "WandbLogger": {"activate": not args.debug, "config": args, "settings": wandb.Settings(_disable_stats=True), **args.wandb}
    })
    ...
    results_dir = f"./models/td3bc/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/dve"
...
def traj_valuation(args):
    ...
    if args.run_dcla_ratio:
        logger = CompositeLogger(
            log_path=f"./ablation_results/td3bc/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/dcla_ratio/{args.dcla_ratio}",
            name=f"td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/td3bc/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/modify_ratio/{args.modify_ratio}/",
            name=f"td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/td3bc/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/std_scale/{args.std_scale}/",
            name=f"td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
    ...
    ###  Train Delta Classifier
    delta = DeltaCla(env.observation_space.shape[0], env.action_space.shape[0], args)
    delta_model_path = f"./models/td3bc/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}"
    ...
    actor_backend_target = MLP(input_dim=obs_shape, hidden_dims=args.hidden_dims)
    actor_target = SquashedDeterministicActor(
        backend=actor_backend_target,
        input_dim=args.hidden_dims[-1],
        output_dim=action_shape,
        device=args.device
    ).to(args.device)
    ...
    policy_target.configure_optimizers(args.actor_lr, args.critic_lr)
    ...
    actor_backend_source = MLP(input_dim=obs_shape, hidden_dims=args.hidden_dims)
    actor_source = SquashedDeterministicActor(
        backend=actor_backend_source,
        input_dim=args.hidden_dims[-1],
        output_dim=action_shape,
        device=args.device
    ).to(args.device)
    ...
    policy_source.configure_optimizers(args.actor_lr, args.critic_lr)
    ...
    if args.run_dcla_ratio:
        np.save(
            logger.log_path + f"/avg_rewards_td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}.npy",
            avg_rewards)
    elif args.run_modify_ratio:
        np.save(
            logger.log_path + f"/avg_rewards_td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}.npy",
            avg_rewards)
    elif args.run_std_scale:
        np.save(
            logger.log_path + f"/avg_rewards_td3bc_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}.npy",
            avg_rewards)

    wandb.finish()

def main(pass_in=None):
    args = parse_args('./reproduce/td3bc/config/offline/cldv/base.py')
    ...
    for sd, td in zip(source_datasets, target_datasets):
        arguments[(sd, td)] = parse_args('./reproduce/td3bc/config/offline/cldv/base.py')
