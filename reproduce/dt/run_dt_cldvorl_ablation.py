...
def vanilla(args):
    ...
    policy.configure_optimizers(lr=args.lr, weight_decay=args.weight_decay, betas=args.betas,
                                warmup_steps=args.warmup_steps)

    ### Log
    exp_name = "_".join([args.d4rl_source_env, args.d4rl_target_env, 'vanilla', "seed"+str(args.seed)])
    logger = CompositeLogger(log_path=f"./results/dt/vanilla", name=exp_name, loggers_config={
    ...
    offline_buffer = D4RLTrajectoryBuffer(mixed_dataset, seq_len=args.seq_len, return_scale=args.return_scale)
    ...
    for i_epoch in trange(1, args.max_epoch + 1):
        ...
        if i_epoch % args.eval_interval == 0:
            eval_metrics = eval_decision_transformer(env, policy, args.target_returns, args.return_scale,
                                                     args.eval_episode, seed=args.seed)
        ...
        if i_epoch % args.save_interval == 0:
            logger.log_object(name=f"policy_{i_epoch}.pt", object=policy.state_dict(),
                              path=f"./models/dt/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/seed{args.seed}/policy")
...
def without_CL(args):
    ...
    logger = CompositeLogger(log_path=f"./results/dt/withoutCL", name=exp_name, loggers_config={
        "FileLogger": {"activate": not args.debug},
        "TensorboardLogger": {"activate": not args.debug},
        "WandbLogger": {"activate": not args.debug, "config": args, "settings": wandb.Settings(_disable_stats=True), **args.wandb}
    })
    ...
    delta_model_path = f"./models/dt/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}"
    ...
    results_dir = f"./models/dt/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/dve"
...
def traj_valuation(args):
    ...
    if args.run_dcla_ratio:
        logger = CompositeLogger(
            log_path=f"./ablation_results/dt/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/dcla_ratio/{args.dcla_ratio}",
            name=f"dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/dt/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/modify_ratio/{args.modify_ratio}/",
            name=f"dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
        ...
        logger = CompositeLogger(
            log_path=f"./ablation_results/dt/{args.d4rl_source_env.split('-')[0]}/{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}/std_scale/{args.std_scale}/",
            name=f"dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}",
            loggers_config={
                "FileLogger": {"activate": not args.debug},
                "TensorboardLogger": {"activate": not args.debug},
                "WandbLogger": {"activate": not args.debug, "config": args,
    ...
    policy_target = DecisionTransformerPolicy(
        dt=dt,
        state_dim=obs_shape,
        action_dim=action_shape,
        seq_len=args.seq_len,
    ...
    policy_target.configure_optimizers(lr=args.lr, weight_decay=args.weight_decay, betas=args.betas,
                                warmup_steps=args.warmup_steps)

    offline_buffer = D4RLTrajectoryBuffer(target_dataset, seq_len=args.seq_len, return_scale=args.return_scale)
    ...
    for i_epoch in trange(1, args.target_max_epoch + 1):
        for i_step in range(args.step_per_epoch):
            batch = next(trainloader_iter)
            train_metrics = policy_target.update(batch, clip_grad=args.clip_grad)

        if i_epoch % args.eval_interval == 0:
            eval_metrics = eval_decision_transformer(env, policy_target, args.target_returns, args.return_scale,
                                                     args.eval_episode, seed=args.seed)
    ...
    def valuate_traj(traj, policy_source, policy_target):
        ...
        #
        # std_scale = 0.1
        # act_mean_source, _, _ = policy_source.actor.sample(obs)
        ...
        # act_mean_target, _, _ = policy_target.actor.sample(obs)
    ...
    policy_source = DecisionTransformerPolicy(
        dt=dt,
        state_dim=obs_shape,
        action_dim=action_shape,
        seq_len=args.seq_len,
    ...
    policy_source.configure_optimizers(lr=args.lr, weight_decay=args.weight_decay, betas=args.betas,
                                       warmup_steps=args.warmup_steps)
    ...
    for e in range(e_size):
        ...
        for i in range(0, len(value_ids), num_trajs):
            ...
            ### add target offline dataset ###
            offline_buffer = D4RLTrajectoryBuffer(source_dataset_sampled, seq_len=args.seq_len, return_scale=args.return_scale)
            ...
            for i_epoch in trange(1, iter_num + 1):
                for i_step in range(args.step_per_epoch):
                    batch = next(trainloader_iter)
                    train_metrics = policy_source.update(batch, clip_grad=args.clip_grad)

                if i_epoch % args.eval_interval == 0:
                    eval_metrics = eval_decision_transformer(env, policy_source, args.target_returns, args.return_scale,
                                                             args.eval_episode, seed=args.seed)
    ...
    if args.run_dcla_ratio:
        np.save(
            logger.log_path + f"/avg_rewards_dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_dcla_ratio_{args.dcla_ratio}_seed_{args.seed}.npy",
            avg_rewards)
    elif args.run_modify_ratio:
        np.save(
            logger.log_path + f"/avg_rewards_dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_modify_ratio_{args.modify_ratio}_seed_{args.seed}.npy",
            avg_rewards)
    elif args.run_std_scale:
        np.save(
            logger.log_path + f"/avg_rewards_dt_{args.d4rl_source_env.split('-')[0]}_{args.d4rl_source_env.split('-')[1]}_{args.d4rl_target_env.split('-')[1]}_std_scale_{args.std_scale}_seed_{args.seed}.npy",
            avg_rewards)
...
def main(pass_in=None):
    args = parse_args('./reproduce/dt/config/offline/cldv/base.py')
    ...
    for sd, td in zip(source_datasets, target_datasets):
        arguments[(sd, td)] = parse_args('./reproduce/dt/config/offline/cldv/base.py')
