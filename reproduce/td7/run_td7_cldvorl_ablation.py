...
def vanilla(args):
    ...
    logger = CompositeLogger(log_path=f"./results/td7/vanilla", name=exp_name, loggers_config={
    ...
    buffer.add_sample({
        k: v[:, None] if k in {"rewards", "terminals"} else v for k, v in mixed_dataset.items() \
        if k in {"observations", "actions", "next_observations", "rewards", "terminals"}
    })
    ...
    for i_epoch in trange(1, args.max_epoch + 1):
        ...
        if i_epoch % args.save_interval == 0:
            logger.log_object(name=f"policy_{i_epoch}.pt", object=policy.state_dict(),
                              path=f"./models/td7/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/seed{args.seed}/policy")
...
def without_CL(args):
    ...
    logger = CompositeLogger(log_path=f"./results/td7/withoutCL", name=exp_name, loggers_config={
        "FileLogger": {"activate": not args.debug},
        "TensorboardLogger": {"activate": not args.debug},
        "WandbLogger": {"activate": not args.debug, "config": args, "settings": wandb.Settings(_disable_stats=True), **args.wandb}
    })
    ...
    results_dir = f"./models/td7/{args.task}/{args.d4rl_source_env}_{args.d4rl_target_env}/dve"
...
def traj_valuation(args):
    ...
    policy_target.configure_optimizers(args.actor_lr, args.critic_lr, args.encoder_lr)
    ...
    buffer.add_sample({
        k: v[:, None] if k in {"rewards", "terminals"} else v for k, v in target_dataset.items() \
        if k in {"observations", "actions", "next_observations", "rewards", "terminals"}
    })
    ...
    for i_epoch in trange(1, args.target_max_epoch + 1):
        for i_step in range(args.step_per_epoch):
            batch, batch_idx = buffer.random_batch(args.batch_size, return_idx=True)
            train_metrics, new_td_error = policy_target.update(batch)
            buffer.batch_update(batch_idx, new_td_error)
    ...
    def valuate_traj(traj, policy_source, policy_target):
        ...
        #
        # std_scale = 0.1
        # zs = policy_source.fixed_encoder.zs(obs)
        # act_mean_source, _, _ = policy_source.actor.sample(obs, zs, True)
        ...
        # zs = policy_target.fixed_encoder.zs(obs)
        # act_mean_target, _, _ = policy_target.actor.sample(obs, zs, True)
    ...
    policy_source.configure_optimizers(args.actor_lr, args.critic_lr, args.encoder_lr)
    ...
    for e in range(e_size):
        ...
        for i in range(0, len(value_ids), num_trajs):
            ...
            buffer.add_sample({
                k: v[:, None] if k in {"rewards", "terminals"} else v for k, v in source_dataset_sampled.items() \
                if k in {"observations", "actions", "next_observations", "rewards", "terminals"}
            })
            ...
            for i_epoch in trange(1, iter_num + 1):
                for i_step in range(args.step_per_epoch):
                    batch, batch_idx = buffer.random_batch(args.batch_size, return_idx=True)
                    train_metrics, new_td_error = policy_source.update(batch)
                    buffer.batch_update(batch_idx, new_td_error)
...
def main(pass_in=None):
    args = parse_args('./reproduce/td7/config/cldv/base.py')
