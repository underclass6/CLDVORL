...
class TD3_GAN(object):
	...
	def train(self, replay_buffer, iterations, batch_size=256, random_s=False, disable_tqdm=True):
		...
		for it in tqdm(range(iterations), disable=disable_tqdm):
			...
			state, demo_action, next_state, reward, not_done = replay_buffer.sample(batch_size, to_device=True)
	...
	def save(self, filename, type):
		torch.save(self.critic.state_dict(), filename + f"_td3_gan_critic_{type}")
		torch.save(self.critic_optimizer.state_dict(), filename + f"_td3_gan_critic_optimizer_{type}")
	...
	def load(self, filename, type):
		self.critic.load_state_dict(torch.load(filename + f"_td3_gan_critic_{type}", map_location=torch.device(device)))
		self.critic_optimizer.load_state_dict(
			torch.load(filename + f"_td3_gan_critic_optimizer_{type}", map_location=torch.device(device)))
		self.critic_target = copy.deepcopy(self.critic)

		self.actor.load_state_dict(torch.load(filename + f"_td3_gan_actor_{type}", map_location=torch.device(device)))
		self.actor_optimizer.load_state_dict(
			torch.load(filename + f"_td3_gan_actor_optimizer_{type}", map_location=torch.device(device)))
		self.actor_target = copy.deepcopy(self.actor)
