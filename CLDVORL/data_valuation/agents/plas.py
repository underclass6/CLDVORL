...
class LatentPerturbation(object):
    ...
    def save(self, filename, type):
        torch.save(self.critic.state_dict(), filename + f"_plas_critic_{type}")
        torch.save(self.critic_optimizer.state_dict(), filename + f"_plas_critic_optimizer_{type}")

        torch.save(self.actor.state_dict(), filename + f"_plas_actor_{type}")
        torch.save(self.actor_optimizer.state_dict(), filename + f"_plas_actor_optimizer_{type}")


    # Load model parameters
    def load(self, filename, type):
        self.critic.load_state_dict(torch.load(filename + f"_plas_critic_{type}", map_location=torch.device(self.device)))
        self.critic_optimizer.load_state_dict(torch.load(filename + f"_plas_critic_optimizer_{type}", map_location=torch.device(self.device)))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(filename + f"_plas_actor_{type}", map_location=torch.device(self.device)))
        self.actor_optimizer.load_state_dict(torch.load(filename + f"_plas_actor_optimizer_{type}", map_location=torch.device(self.device)))
        self.actor_target = copy.deepcopy(self.actor)
