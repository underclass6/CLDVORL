...
class BEAR(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, disable_tqdm=False):
        ...
        for it in tqdm(range(iterations), disable=disable_tqdm):
            state, action, next_state, reward, not_done, label = replay_buffer.sample(batch_size, to_device=True)
    ...
    def load(self, filename, type):
        ...
        self.critic_optimizer.load_state_dict(torch.load(filename + f"_bear_critic_optimizer_{type}", map_location=torch.device(device)))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(filename + f"_bear_actor_{type}", map_location=torch.device(device)))
        self.actor_optimizer.load_state_dict(torch.load(filename + f"_bear_actor_optimizer_{type}", map_location=torch.device(device)))
...
class BEAR_IS(object):
    ...
    # Save model parameters
    def save(self, filename, type):
        torch.save(self.critic.state_dict(), filename + f"_bear_IS_critic_{type}")
        torch.save(self.critic_optimizer.state_dict(), filename + f"_bear_IS_critic_optimizer_{type}")

        torch.save(self.actor.state_dict(), filename + f"_bear_IS_actor_{type}")
        torch.save(self.actor_optimizer.state_dict(), filename + f"_bear_IS_actor_optimizer_{type}")


    # Load model parameters
    def load(self, filename, type):
        self.critic.load_state_dict(torch.load(filename + f"_bear_IS_critic_{type}", map_location=torch.device(device)))
        self.critic_optimizer.load_state_dict(torch.load(filename + f"_bear_IS_critic_optimizer_{type}", map_location=torch.device(device)))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(filename + f"_bear_IS_actor_{type}", map_location=torch.device(device)))
        self.actor_optimizer.load_state_dict(torch.load(filename + f"_bear_IS_actor_optimizer_{type}", map_location=torch.device(device)))
        self.policy_target = copy.deepcopy(self.actor)
