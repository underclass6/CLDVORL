...
from param_dvbcq import args


env = utils.get_gym(args.env, args.target_env_friction, args.target_env_mass_torso)
args_dv.cuda = True
delta = DeltaCla(env.observation_space.shape[0], env.action_space.shape[0], args_dv)

# print('Start training delta classifier...')
# delta.train(rbuf_source, rbuf_target, args_dv)
# delta.save_delta_models('delta')
# print('Finished!')

print('Loading delta classifier...')
delta.load_delta_models('delta')
print('Finished')
...
class BCQ(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, random_s=False, disable_tqdm=True):
        ...
        for it in tqdm(range(iterations), disable=disable_tqdm):
            ...
            importance_weight = torch.cuda.FloatTensor(np.sqrt(np.exp(delta.delta_dynamic(state, action, next_state))))
            importance_weight = importance_weight.reshape(batch_size, -1)
            ...
            current_Q1, current_Q2 = self.critic(state, action)
            current_Q1 = current_Q1 * importance_weight
            current_Q2 = current_Q2 * importance_weight
            target_Q = target_Q * importance_weight
            critic_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)
