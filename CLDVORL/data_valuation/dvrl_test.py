...
# from data_valuation.agents.bcq_test import BCQ
...
from delta_classifier import DeltaCla
from param_data_valuation import args as args_dv
...
class DVRL(object):
    def __init__(self, source_dataset, target_dataset, device, target_env, results_dir, ex_configs, args={}):
        ...
        args_dv.cuda = True
        self.delta = DeltaCla(target_env.observation_space.shape[0], target_env.action_space.shape[0], args_dv)
    ...
    def train(self):
        ...
        # for iteration in tqdm(range(self.source_dataset.trajs_size)):
            ...
            # --------------------------------------baseline1-----------------------------------------------------------
            # delta_avg = -np.mean(np.abs(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states))))
            # delta_avg = -np.mean(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states)))
            # delta_avg = -np.sum(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states)))
            # delta_avg = -np.sum(np.abs(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states))))
            # delta_avg = -np.sum(np.abs(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states)))) + 0.01*len(states)
            ...
            # delta_avg = -np.sum(est_dv_curr.squeeze(1).detach().cpu().numpy() * np.abs(self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states)))) + 10*np.sum(est_dv_curr.squeeze(1).detach().cpu().numpy())
            # reinforce_reward = np.exp(delta_avg)
            ...
            # reinforce_reward = dvbcq_avg_rew - delta_avg

            # reinforce_rewards = np.append(reinforce_rewards, reinforce_reward)
            # reinforce_rewards = reinforce_rewards[:]
            # reinforce_reward = (reinforce_reward - np.mean(reinforce_rewards)) / (np.std(reinforce_rewards) + 1e-8)
            # dvbcq_avg_rew = reinforce_reward
            # --------------------------------------baseline1-----------------------------------------------------------

            # --------------------------------------baseline2-----------------------------------------------------------
            # data = ReplayBuffer(self.state_dim, self.action_dim, self.device)
            # rewards = self.delta.delta_dynamic(torch.cuda.FloatTensor(states), torch.cuda.FloatTensor(actions), torch.cuda.FloatTensor(next_states))
            # for i in range(self.dict['batch_size']):
            #     data.add(states[i], actions[i], next_states[i], rewards[i], 1.0 - terminals[i])
            # dv_reward = value_of_data_bcq(data, self.rl_model)
            #
            # reinforce_reward = dv_reward - dv_avg_rew
            # dv_avg_rew = (self.dict['T'] - 1) * dv_avg_rew / self.dict['T'] + dv_reward / self.dict['T']
            #
            # reinforce_rewards = np.append(reinforce_rewards, reinforce_reward)
            # reinforce_reward = (reinforce_reward - np.mean(reinforce_rewards)) / (np.std(reinforce_rewards) + 1e-8)
            # --------------------------------------baseline2-----------------------------------------------------------
